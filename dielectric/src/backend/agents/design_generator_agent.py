        ...
        for comp in design_data["components"]:
            ...
            if check_jlcpcb:
                ...
                if jlcpcb_info:
                    ...
                    logger.info(f"   JLCPCB: {comp.get('name')} ({package}) - Stock: {jlcpcb_info.get('stock', 0)}, Price: ${jlcpcb_info.get('price', 0):.4f}")
                else:
                    logger.debug(f"   JLCPCB: No match found for {comp.get('name')} ({package})")
