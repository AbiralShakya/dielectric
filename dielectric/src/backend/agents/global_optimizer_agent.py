...
Now includes research-backed global optimization algorithms.
"""
...
try:
    ...
    from backend.optimization.global_layout_optimizer import GlobalLayoutOptimizer, GlobalOptimizationMethod
    ...
    try:
        from src.backend.optimization.global_layout_optimizer import GlobalLayoutOptimizer, GlobalOptimizationMethod
    except ImportError:
        GlobalLayoutOptimizer = None
        GlobalOptimizationMethod = None
    try:
...
class GlobalOptimizerAgent:
    ...
    async def _standard_global_optimize(
        ...
        Uses research-backed algorithms:
        ...
        # Use research-backed global optimizer if available
        if GlobalLayoutOptimizer:
            method_str = plan.get("method", "hybrid")
            try:
                method = GlobalOptimizationMethod[method_str.upper()] if GlobalOptimizationMethod else GlobalOptimizationMethod.HYBRID
            except:
                method = GlobalOptimizationMethod.HYBRID if GlobalOptimizationMethod else None
            
            if method:
                logger.info(f"   Using Global Layout Optimizer: {method.value}")
                global_optimizer = GlobalLayoutOptimizer(
                    scorer=self.scorer.base_scorer if hasattr(self.scorer, 'base_scorer') else WorldModelScorer(ScoreWeights(
                        alpha=weights.get("alpha", 0.3),
                        beta=weights.get("beta", 0.3),
                        gamma=weights.get("gamma", 0.2),
                    ...
                    method=method,
                    max_iterations=plan.get("max_iterations", 1000),
                    convergence_threshold=0.001
                )
                
                best_placement, best_score, stats = global_optimizer.optimize(
                    placement,
                    weights,
                    timeout=timeout,
                    callback=callback
                ...
                stats["method"] = f"global_{method.value}"
                return {
                    "success": True,
        ...
        logger.info("   Using Simulated Annealing (fallback)")
